...
import { validateDocument, validateEmail, validatePhone } from "@/lib/validation"
import { CalendarDays, ShoppingCart, CreditCard, CheckCircle, Bike, Plus, Minus, Package } from "lucide-react"
...
type Step = "dates" | "bikes" | "accessories" | "customer" | "payment" | "confirmation"
...
const translateBikeContent = (textObject: { es: string; en: string; nl: string }, language: string): string => {
...
export default function ReservePage() {
  ...
  useEffect(() => {
    const fetchAccessories = async () => {
      ...
      try {
        const { data } = await supabase.from("accessories").select("*").eq("available", true)
        if (data) {
          setAccessories(data)
        }
      } finally {
  ...
  const fetchAvailableBikes = async () => {
    ...
    try {
      const { data: allBikes } = await supabase.from("bikes").select("*").eq("available", true)
      ...
      if (allBikes) {
        ...
        overlappingReservations?.forEach((reservation) => {
          reservation.bikes.forEach((bike: any) => {
            bike.bikes?.forEach((b: any) => reservedBikeIds.add(b.id))
          })
        })
  ...
  const groupBikesByModel = () => {
    const grouped = availableBikes.reduce((acc: Record<string, BikeModel>, bike) => {
      ...
      const existingSizeIndex = acc[key].availableSizes.findIndex((s) => s.size === bike.size)
  ...
  const handleBikeSelection = (model: BikeModel, size: string, quantity: number) => {
    const existingIndex = selectedBikes.findIndex((bike) => bike.title_es === model.title_es && bike.size === size)

    if (quantity === 0) {
      if (existingIndex >= 0) {
        setSelectedBikes(selectedBikes.filter((_, index) => index !== existingIndex))
      }
      return
    }
  ...
  const handleAccessorySelection = (accessory: Accessory) => {
    const existingIndex = selectedAccessories.findIndex((a) => a.id === accessory.id)

    if (existingIndex >= 0) {
      setSelectedAccessories(selectedAccessories.filter((_, index) => index !== existingIndex))
    } else {
  ...
  const calculateTotal = (): number => {
    ...
    const days = calculateDays(formatDate(startDate), formatDate(endDate))
    ...
    const totalBikeCount = selectedBikes.reduce((total, bike) => total + bike.quantity, 0)
  ...
  const isDateDisabled = (date: Date): boolean => {
    ...
    if (isSunday(date)) return true // Disable Sundays
    return false
  }
  ...
  const handleSubmitReservation = async (paymentIntentId?: string) => {
    if (!startDate || !endDate) return
    ...
    try {
      const reservationData = {
        ...
        total_days: calculateDays(formatDate(startDate), formatDate(endDate)),
        ...
        status: paymentIntentId ? "confirmed" : isAdminMode ? "confirmed" : "pending",
      ...
      if (data) {
        ...
        if (!isAdminMode) {
          await sendConfirmationEmail({ ...data, ...reservationData })
        }
        ...
        return (
          <Card>
            ...
            <CardContent>
              ...
              {startDate && endDate && (
                <div className="mt-6 p-4 bg-green-50 rounded-lg">
                  <p className="text-sm text-green-800">
                    <strong>{t("duration")}:</strong> {calculateDays(formatDate(startDate), formatDate(endDate))}{" "}
                    {t("days")}
                  </p>
                  <p className="text-sm text-green-800">
                    <strong>{t("from")}:</strong> {startDate.toLocaleDateString()} <strong>{t("to")}:</strong>{" "}
              ...
              <div className="mt-6">
                <Button onClick={() => setCurrentStep("bikes")} disabled={!startDate || !endDate} className="w-full">
                  {t("continue")}
                </Button>
              </div>
            </CardContent>
                ...
                <>
                  {Object.entries(groupedModels).map(([category, models]) => (
                    <div key={category} className="mb-8">
                      <h3 className="text-lg font-semibold mb-4">{getCategoryName(category as BikeCategory)}</h3>

                      {models.map((model, modelIndex) => (
                        <div key={`${model.title_es}-${modelIndex}`} className="mb-6 p-4 border rounded-lg">
                          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-sm font-medium text-green-600">
                                  {calculatePrice(
                                    model.category,
                                    calculateDays(formatDate(startDate!), formatDate(endDate!)),
                                  )}
                                  {t("euro")}
                                  {t("perDay")}
                                </p>
                            ...
                            <div className="lg:col-span-2">
                              <Label className="text-sm font-medium mb-2 block">{t("availableSize")}</Label>
                              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                {model.availableSizes.map((sizeInfo) => {
                                  const selectedBike = selectedBikes.find(
                                    (b) => b.title_es === model.title_es && b.size === sizeInfo.size,
                                  )
                                  const currentQuantity = selectedBike?.quantity || 0

                                  return (
                                    <div key={sizeInfo.size} className="border rounded-lg p-3">
                                      ...
                                      <div className="flex items-center gap-2">
                                        <Button
                                          ...
                                          variant="outline"
                                          onClick={() =>
                                            handleBikeSelection(model, sizeInfo.size, Math.max(0, currentQuantity - 1))
                                          }
                                        ...
                                        <Button
                                          size="sm"
                                          variant="outline"
                                          onClick={() => handleBikeSelection(model, sizeInfo.size, currentQuantity + 1)}
                                          disabled={currentQuantity >= sizeInfo.count}
                                        >
                  ...
                  {selectedBikes.length > 0 && (
                    <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                      <h4 className="font-semibold mb-2">{t("selectedBikes")}:</h4>
                      {selectedBikes.map((bike, index) => (
                        <p key={index} className="text-sm">
                          {translateBikeContent(
                            { es: bike.title_es, en: bike.title_en, nl: bike.title_nl },
                            language
            ...
            <CardContent>
              {isLoadingAccessories ? (
                ...
                <div className="space-y-6">
                  <div>
                    <h4 className="font-semibold mb-4">{t("availableAccessories")}</h4>
  ...
  <div className="flex items-center space-x-2">
    ...
    <Label htmlFor="insurance" className="font-medium">
      ...
      {t("euro")} {t("perDay")} {t("perBike")} (max {INSURANCE_MAX_PRICE}
      {t("euro")} {t("perBike")})
    </Label>
  </div>
  <p className="text-sm text-gray-600 mt-1">{t("completeProtectionDamage")}</p>
</div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-1 text-sm">
                      <div className="flex justify-between">
                        <span>
                          {t("bikes")} ({selectedBikes.reduce((total, bike) => total + bike.quantity, 0)})
                        </span>
                        <span>
                          {selectedBikes.reduce(
                            (total, bike) =>
                              total +
                              calculatePrice(bike.category, calculateDays(formatDate(startDate!), formatDate(endDate!))) *
                                bike.quantity,
                            0,
                          )}
                          {t("euro")}
                        ...
                        <div className="flex justify-between">
                          ...
                          <span>
                            {selectedAccessories.reduce(
                              (total, acc) =>
                                total + acc.price * calculateDays(formatDate(startDate!), formatDate(endDate!)),
                              0,
                            )}
                            {t("euro")}
                          </span>
                      ...
                      {hasInsurance && (
                        <div className="flex justify-between">
                          ...
                          <span>
                            {Math.min(
                              INSURANCE_PRICE_PER_DAY * calculateDays(formatDate(startDate!), formatDate(endDate!)),
                              INSURANCE_MAX_PRICE
                  ...
                  <div className="mt-6 flex gap-4">
                    <Button variant="outline" onClick={() => setCurrentStep("bikes")}>
                      {t("back")}
                    </Button>
                    <Button onClick={() => setCurrentStep("customer")} className="flex-1">
                      {t("continue")}
                    </Button>
                  </div>
                </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  />
                  {validationErrors.dni && <p className="text-red-500 text-sm mt-1">{validationErrors.dni}</p>}
                  <p className="text-xs text-gray-500 mt-1">{t("validationValidDocumentInfo")}</p>
                </div>
              </div>
              ...
              <div className="mt-6 flex gap-4">
                ...
                >
                  {isSubmitting ? t("processing") : isAdminMode ? t("reservationCreate") : t("continuePayment")}
                </Button>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="space-y-6">
            <div className="bg-yellow-50 p-4 rounded-lg">
              <p className="text-sm text-yellow-800">
                <strong>{t("important")}:</strong> {t("depositMessage", { amount: calculateTotalDeposit() })}
              </p>
            </div>
            ...
            <StripePayment
              ...
              onSuccess={(paymentIntentId) => {
                handleSubmitReservation(paymentIntentId)
              }}
              onError={(error) => {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-4">
            {t("reserveBikes")} {isAdminMode && <Badge variant="secondary">Modo Admin</Badge>}
          </h1>
          ...
          <div className="flex items-center flex-nowrap gap-2 md:gap-4 mb-8 overflow-x-auto sm:overflow-x-visible sm:justify-between">
            {[
              ...
              ...(isAdminMode ? [] : [{ key: "payment", label: t("payment"), icon: CreditCard }]),
              ...
              return (
                <div key={step.key} className="flex items-center flex-shrink-0 min-w-[120px]">
                  <div
                  ...
                  <span
                    className={`ml-2 text-xs sm:text-sm whitespace-nowrap ${
                      isActive ? "text-green-600 font-medium" : isCompleted ? "text-green-600" : "text-gray-400"
                    }`}
                  >
                    {step.label}
                  </span>
                  {index < array.length - 1 && (
                    <div className="hidden sm:block w-4 h-0.5 mx-2 bg-gray-200 sm:w-8 sm:mx-4 sm:bg-gray-200" style={{ minWidth: 8 }} />
                  )}
                </div>
              )
            })}
